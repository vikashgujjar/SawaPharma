...
const ProductTable = () => {
  const [betaLactamProducts, setBetaLactamProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    const fetchBetaLactamProducts = async () => {
      try {
        const response = await fetch(`${baseLink}/capsule`);
        if (!response.ok) {
          throw new Error("Failed to fetch data");
        }
        const data = await response.json();
        setBetaLactamProducts(data);
      } catch (err) {
      ...
      <div className="container mx-auto py-20 px-5 lg:px-28">
        <div className="border mb-1 flex gap-5 px-5 py-2 justify-between">
          <h1 className="text-sm md:text-2xl py-3 font-semibold text-center">
            SAWA PHARMA INDIA PVT. LTD. TABLETS & CAPSULES 
          </h1>
          {product.map(
            (product) =>
              product.id === 5 && (
        ...
        <div className="overflow-x-auto">
          <table className="table-auto w-full border-collapse border border-gray-300 shadow-lg rounded-lg">
            ...
            {loading ? (
              ...
              <tbody>
                {betaLactamProducts.map((product, index) => (
                  <tr
                    key={product.id}
                    className={`${
